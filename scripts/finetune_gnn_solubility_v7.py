"""
v7: PKa pre-trained GNN + supplementary data + solubility fine-tuning.
Combines the two best strategies: pre-trained backbone + real experimental data.
"""
...
# ── 6. Load pre-trained pKa backbone ──
...
# ── 7. Fine-tune on solubility + supplementary ──
print("\n" + "=" * 50)
print("Fine-tuning GNN (pKa backbone + supplementary data)...")
print("=" * 50)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if vl < best_val_loss - 1e-5:
        ...
        best_epoch = epoch
        patience_counter = 0
        save_gnn_model(model, "output_v2/gnn_solubility_model_v7.pt")
    else:
        patience_counter += 1
...
# ── 8. 4-way comparison on test_batch ──
print("\n" + "=" * 50)
print("4-way comparison on test_batch.csv (v2 vs v3 vs v6 vs v7)")
print("=" * 50)
...
gnn_v6 = load_gnn_model("output_v2/gnn_solubility_model_v6.pt", device=DEVICE)
gnn_v7 = model
...
for smi in df_test["SMILES"]:
    ...
    with torch.no_grad():
        ...
        g7p = float(gnn_v7(encoder.mol_to_graph(mol)).item())

    rows.append({
        "SMILES": smi,
        "RF_v2": rf2,
        "GNN_v2": g2p, "GNN_v3": g3p, "GNN_v6": g6p, "GNN_v7": g7p,
        "|RF-GNN|_v2": abs(rf2 - g2p),
        "|RF-GNN|_v3": abs(rf2 - g3p),
        "|RF-GNN|_v6": abs(rf2 - g6p),
        "|RF-GNN|_v7": abs(rf2 - g7p),
    })

res = pd.DataFrame(rows)
print(f"\n  {'Model':<20} {'Mean |RF-GNN|':>15} {'Reduction':>15}")
print(f"  {'-'*50}")
for ver in ["v2", "v3", "v6", "v7"]:
    col = f"|RF-GNN|_{ver}"
    red = res['|RF-GNN|_v2'].mean() - res[col].mean()
    pct = red / res['|RF-GNN|_v2'].mean() * 100
    print(f"  {'GNN ' + ver:<20} {res[col].mean():>15.3f} {red:+>13.3f} ({pct:+.1f}%)")

print(f"\n  Hard cases:")
for _, r in res.sort_values("|RF-GNN|_v2", ascending=False).head(5).iterrows():
    smi_short = r["SMILES"][:35]
    print(f"    {smi_short:37s} v2={r['GNN_v2']:+.2f} v3={r['GNN_v3']:+.2f} v7={r['GNN_v7']:+.2f} |RF|={r['RF_v2']:+.2f}")

print("\nModel saved: output_v2/gnn_solubility_model_v7.pt")
